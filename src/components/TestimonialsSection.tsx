...
const TestimonialsSection = () => {
  ...
  return <section ref={sectionRef} className="py-24 md:py-32 bg-secondary relative overflow-hidden" id="testimonials">
      {/* Parallax decorative elements */}
      <div className="absolute top-0 left-1/4 w-64 h-64 bg-foreground/[0.02] rounded-full blur-3xl" style={{
      transform: `translateY(${scrollProgress * 80}px)`
    }} />
      <div className="absolute bottom-0 right-1/4 w-48 h-48 bg-foreground/[0.02] rounded-full blur-2xl" style={{
      transform: `translateY(${scrollProgress * -60}px)`
    }} />
      
      {/* 3D floating shapes */}
      <div className="absolute top-20 right-20 w-16 h-16 border border-foreground/5 rounded-2xl float-3d" />
      <div className="absolute bottom-32 left-16 w-20 h-20 border border-foreground/5 rounded-full float-3d" style={{
      animationDelay: '2s'
    }} />
      ...
      <div className="section-container relative z-10">
        <AnimatedSection>
          <div className="text-center mb-16">
            ...
            <h2 className="section-title font-sans">What our clients say <span className="font-serif italic font-normal"> say</span>
            </h2>
          </div>
        </AnimatedSection>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => <ScrollTriggered3DCard key={index} delay={index * 150}>
