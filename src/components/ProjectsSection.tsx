...
const ProjectsSection = () => {
  ...
  return (
    <section ref={sectionRef} className="py-24 md:py-32 bg-secondary relative overflow-hidden" id="projects">
      ...
      {/* 3D floating shapes */}
      <div className="absolute top-32 left-10 w-20 h-20 border border-foreground/5 rounded-2xl float-3d" style={{ animationDelay: '0s' }} />
      <div className="absolute top-1/2 right-20 w-16 h-16 border border-foreground/5 rounded-full float-3d" style={{ animationDelay: '2s' }} />
      ...
      <div className="section-container relative z-10">
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map((category) => (
                <button
                  ...
                  className={cn(
                    "px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 tilt-hover",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.map((project, index) => (
            <ScrollTriggered3DCard key={project.id} delay={index * 100}>
              <Link 
                to={`/project/${project.id}`}
                className="group block bg-background rounded-2xl overflow-hidden transition-all duration-500 h-full glow-effect"
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag, tagIndex) => (
                      <span 
                        key={tag}
                        className="text-xs px-3 py-1 rounded-full bg-secondary text-muted-foreground micro-interaction"
