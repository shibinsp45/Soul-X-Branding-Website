...
const Navbar = () => {
  ...
  return <header className={cn("fixed top-0 left-0 right-0 z-50 py-4 md:py-5 transition-all duration-500", isScrolled ? "bg-background/90 backdrop-blur-md border-b border-border" : "bg-transparent")}>
      ...
      <div className={cn("fixed inset-0 z-40 bg-background flex flex-col pt-20 px-6 md:hidden transition-all duration-300 ease-in-out", isMenuOpen ? "opacity-100 translate-x-0" : "opacity-0 translate-x-full pointer-events-none")}>
        <nav className="flex flex-col space-y-6 items-center mt-8">
          {navItems.map(item => <a key={item.label} href={item.href} className="text-2xl font-display font-medium py-2 text-foreground" onClick={e => {
          if (item.href === '#') {
            e.preventDefault();
            scrollToTop();
          }
